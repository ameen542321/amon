...
class AuthTokenController extends Controller
{
    ...
    public function login(Request $request): JsonResponse
    {
        ...
        [$plainToken, $token] = DB::transaction(function () use ($actor, $validated, $request): array {
            $plainToken = 'carled_'.Str::random(64);
            $identity = [
                'actor_type' => $this->access->actorType($actor),
                'actor_id' => (int) $actor->getAuthIdentifier(),
            ];

            ApiAccessToken::query()->where($identity)->where('device_uuid', $validated['device_uuid'])
            ...
            $token = ApiAccessToken::query()->create([
                ...
                'abilities' => ['app:read', 'devices:manage', 'notifications:read', 'notifications:write'],
                'last_ip' => $request->ip(),
            ...
            $excess = ApiAccessToken::query()->where($identity)->whereNull('revoked_at')->where('expires_at', '>', now())
                ->orderByDesc('last_used_at')->pluck('id')
                ->slice(max(1, config('mobile_api.max_devices_per_account')));
            if ($excess->isNotEmpty()) ApiAccessToken::query()->whereKey($excess)->update(['revoked_at' => now()]);
    ...
    {
        $request->attributes->get('api_access_token')->forceFill(['revoked_at' => now()])->save();
