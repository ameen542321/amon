...
يعتمد الصادر على `request_business_date`، والوارد المعالج على `action_business_date`، والوارد المعلق على يوم الإرسال مؤقتًا. للسجلات القديمة التي لا تملك يوم عمل محفوظًا، يرجع API إلى تاريخ العملية الفعلي ويصرح بالمصدر داخل `business_date_source`. تعتمد أسماء المنتجات والوحدات على اللقطات المحفوظة، ولا تعرض أرصدة المخزون الداخلية قبل النقل وبعده.
