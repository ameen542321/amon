...
تتطلب المسارات Bearer Token وصلاحية `purchase-orders:read`، ويتحقق الخادم من حق الحساب في المتجر. تدعم القائمة `status` و`supplier` و`from` و`to` و`limit` و`cursor`.
