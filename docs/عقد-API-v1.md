...
أعمدة تواريخ الجلسة المدارة من التطبيق تستخدم `DATETIME` لضمان التوافق مع MySQL
وMariaDB اللذين قد يرفضان `TIMESTAMP NOT NULL` بلا default ضمني.


## كتالوج المنتجات

مسارات `/api/v1/catalog/*` للقراءة فقط وتتطلب `catalog:read` و`store_id` مصرحًا.
القائمة تستخدم Cursor، ولا تعيد تكلفة المنتج. المزامنة تستخدم نافذة `since`/`until`
ثابتة وTombstones للحذف. التفاصيل التشغيلية في
[`تشغيل-API-كتالوج-المنتجات.md`](تشغيل-API-كتالوج-المنتجات.md).


## المخزون وحركاته

مسارات `/api/v1/inventory/*` للقراءة فقط وتتطلب `inventory:read` و`store_id` مصرحًا.
تفاصيل العقد والتدقيق في [`تشغيل-API-المخزون-والحركات.md`](تشغيل-API-المخزون-والحركات.md).


## الشفت ويوم العمل

مسارات `/api/v1/shifts/*` للقراءة فقط وتتطلب `shifts:read`. المرجع التشغيلي:
[`تشغيل-API-الشفت-ويوم-العمل.md`](تشغيل-API-الشفت-ويوم-العمل.md).


## المبيعات والفواتير

مسارات `/api/v1/sales/*` للقراءة فقط وتتطلب `sales:read`. المرجع:
[`تشغيل-API-المبيعات-والفواتير.md`](تشغيل-API-المبيعات-والفواتير.md).
